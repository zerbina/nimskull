...
proc `=`*[T](dest: var T; src: T) {.noSideEffect, inline, magic: "Asgn".}
proc `=copy`*[T](dest: var T; src: T) {.noSideEffect, inline, magic: "Asgn".}
...
when true:
  proc finished*[T: proc](x: T): bool {.noSideEffect, magic: "Finished".} =
    ...
    # note: the actual logic is generated by the compiler's
    # code-generator -- the implementation here is only used to reject
    # non-closure-iterator procedures
    when T isnot "iterator":
      {.error: "`finished` is only allowed with closure iterators".}
